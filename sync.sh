...
LOCAL_DIR="."
REMOTE_DIR="imacdowell@192.168.1.183:./docker-nginx-ts/"
...
case "$1" in
    "up")
        ...
        rsync -avz --checksum --ignore-times --update --exclude='.git' "$LOCAL_DIR" "$REMOTE_DIR"
        ;;
    "down")
        echo "rsync: $REMOTE_DIR To: $LOCAL_DIR"
        rsync -avz --checksum --ignore-times --update --exclude='.git' "$REMOTE_DIR" "$LOCAL_DIR"
        ;;
    *)
